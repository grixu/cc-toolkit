...
- **No path arguments** → review the current branch diff. Detect what exists with the
  ...
  ```bash
  python3 ${CLAUDE_PLUGIN_ROOT}/skills/comment-review/scripts/get_changes.py --scope uncommitted
  python3 ${CLAUDE_PLUGIN_ROOT}/skills/comment-review/scripts/get_changes.py --scope committed
  ```
...
`${CLAUDE_PLUGIN_ROOT}/skills/comment-review/references/rules.md` completely
...
in `references/rules.md` — consult it per comment rather than relying on this map.

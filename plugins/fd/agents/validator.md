...
description: >-
  ...
  Returns a structured per-check result plus human-only doubts. Internal sub-agent
  ...
  assistant: "Reading spec.md and ac-map.json fresh from disk; returning {dimension:'coverage', checks:[{id, verdict, evidence}...], doubts:[]} — every check pass or fail, no warnings."
...
3. Decide each check **binary: pass or fail.** There is no third state. Do not soften a fail
   ...
   judgement call rather than a mechanical fail, record it as a `doubt`, not a silent pass.
...
- **structural** — FR/NFR do not contradict; contracts are complete (tables, enums, state
  ...
  `idCounters` is not silently a new kind — record it as a `doubt` for the command to resolve
  with a human (accept the new `KIND` → add to `idCounters`, or fix the typo).
...
```json
{
  ...
  "doubts": ["<a question only a human can answer>"]
}
```

Use stable, dimension-prefixed check ids (e.g. `coverage.ac-single-behavior`) so the command
can track a waiver against the same `checkId` across re-validations. Report every check —

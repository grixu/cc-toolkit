...
Task agents run as the plugin's **`fd:implementer`** subagent — a restricted toolset (native
file/shell tools plus the supported MCP servers: `context7`, `firecrawl`, `codebase-memory-mcp`)
so each agent's fixed startup context stays small (a field run measured 43–92k tokens of
inherited-everything baseline vs 21k for the restricted merger, re-read on every one of ~90 calls
per agent). Mechanical stages — worktree prep, the merger, the smoke — run at **low reasoning
effort**. The main thread keeps: all preconditions, building the run args, persisting state at run
...
- The task file is **self-contained** — do not re-grep or rediscover paths, symbols, or contracts
  already named in its body or `codeDeps`. Read it once and trust it. It is the agent's **only fd
  material**: never read `spec.md`, other tasks' files, or feature workspace state — a gap in the
  task file is a diagnosis or escalation to report, not a license to hunt (a round-3 field run had
  agents read the whole spec and four peer task files).
- **Code retrieval:** with `graphMcp: true` (i.e. `codebase-memory-mcp` in `mcp.detected`), locate
  and fetch code through the graph — `search_graph`/`search_code`, `get_code_snippet`,
  `trace_path` — instead of Grep/Glob or shell `cat | grep` sweeps; Read stays for files being
  edited, `codeDeps`, and configs. The graph indexes the repo-root checkout, so the worktree's own
  uncommitted work needs Read/git, not the graph.

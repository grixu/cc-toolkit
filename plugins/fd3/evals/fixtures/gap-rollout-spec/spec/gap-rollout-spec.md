...
**Declared gap — the production write ceiling of the ledger database is unconfirmed.** Nobody in
this session could establish how many settlement writes per second the shared staging instance
sustains before the ledger's connection pool saturates, and no measurement exists. Owner: the
platform team. Placement: a gate before phase 2 — the flag flip in CONFIG-1 is what puts real
write volume on the table, so the ceiling must be measured and recorded before that phase starts.
The phase-1 elements are unaffected: they land dark and write nothing.
...
| The ledger write ceiling is unmeasured | no measurement exists; declared as a gap in section 7 with the platform team as owner and a gate before phase 2 as placement |
| Verdict | phase 1: yes; phase 2: yes, behind the declared gap's gate — spec is ready to split |

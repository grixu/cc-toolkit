...
### API-3 — ledger entry export (ledger service)

`GET /ledger/entries/export?orderId=` streams the entries for an order as CSV.

- Request: `orderId` query parameter, required, non-empty string.
- Response: `text/csv` with columns `order_id, amount_minor, direction, created_at`.
- Errors: 400 on a missing or empty `orderId`; 200 with only the header row when no entries exist.
- Auth: the existing internal service token middleware.
- Limits: response capped at 10000 rows.
